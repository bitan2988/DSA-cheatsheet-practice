...
// Another approach is to fill all the even positions of the result string first, with the highest frequency character. 
...
// returns the character with highest frequency
...
string rearrangeString(string S)
{
    ...
    // making a count array storinf the frequency
	...
	for (int i = 0; i < 26; i++) {
		while (count[i] > 0) {
			ind = (ind >= n) ? 1 : ind;  // checking if all the even positions are filled or not
			res[ind] = 'a' + i;
			ind += 2;
			count[i]--;
		}
