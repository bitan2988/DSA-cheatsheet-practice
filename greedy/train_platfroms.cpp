...
// declaring vector of pairs for platform
    std::vector<std::pair<int, int> > vect[n + 1];

// Entering values in vector of pairs as per platform number make departure time first element  of pair
    for (int i = 0; i < m; i++)
        vect[arr[i][2]].push_back(std::make_pair(arr[i][1], arr[i][0]));        //vector index represent the platform stroing the value {departure_time, arrival_time}
...
int main()
{
    // [arrival_time, departture_time, platform]
    ...
    std::cout<<"ANSWER = "<<maxStop(arr)<<std::endl;
    return 0;
}