...
RED BLACK TREE
--------------
	EVERY NODE HAS TO EITHER RED/BLACK (1 BIT TO STORE RED/BLACK)
	THE ROOT IS ALWAYS BLACK
	NEW INSERIONS ARE ALWAYS RED
	EVERY PATH FROM ROOT-LEAF HAS THE SAME NUMBER OF BLACK NODES
	NO PATH CAN HAVE CONSECUTIVE RED NODES (IF PARENT IS RED THEN CHILD CANT BE RED) BUT CAN HAVE TWO CONSECUTIVE BLACK NODES
	NULLS ARE BLACK

* AVL TREE  IS USED WHEN WE HAVE MORE OF SEARCHING OPERATION AS IT GAURANTEE A SEARCH TIME OF O(logN) BUT RBT HAS THIS TIME AS AVG CASE AND DOESNT GAURANTEE
  RBT IS TO BE USED WHERE WE HAVE MOE OF INSERTION/DELETION AS IT REQUIRES AT-MOST 2 ROTATIONS OR RECOLOURING TO BALANCE THE TREE i.e O(logN)

  * AVL - strictly height balanced
  * RBT - roughly height balanced

  * AVL trees are subset of RBT

  * every perfect binary tree with all black-nodes is a RBT
  * length of LONGEST_PATH <= 2*(length of SHORTEST_PATH)
